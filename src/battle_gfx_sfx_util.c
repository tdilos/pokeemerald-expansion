...
void InitAndLaunchChosenStatusAnimation(enum BattlerId battler, bool32 isVolatile, u32 status)
{
    ...
    if (!isVolatile)
    {
        if (status == STATUS1_FREEZE)
            ...
            LaunchStatusAnimation(battler, B_ANIM_STATUS_INF);
		else if (status == STATUS1_DAZE)
            LaunchStatusAnimation(battler, B_ANIM_STATUS_DAZ);
		else if (status == STATUS1_EXHAUST)
            LaunchStatusAnimation(battler, B_ANIM_STATUS_EXH);
		else if (status == STATUS1_FEAR)
            LaunchStatusAnimation(battler, B_ANIM_STATUS_FER);
        else // no animation
