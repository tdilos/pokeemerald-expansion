...
static bool32 CursorIsNextToBlank(void)
{
    ...
    s8 col0 = MAX(cursor->sCol - 1, FIRST_SLIDING_PUZZLE_COL);
	s8 row1 = MIN(cursor->sRow + 1, FINAL_SLIDING_PUZZLE_ROW);
    s8 col1 = MIN(cursor->sCol + 1, FINAL_SLIDING_PUZZLE_COL);
	
    if (sSlidingPuzzle->tiles[row0][cursor->sCol] == __ && row0 != FIRST_SLIDING_PUZZLE_ROW)
        return TRUE;
    if (sSlidingPuzzle->tiles[cursor->sRow][col0] == __ && col0 != FIRST_SLIDING_PUZZLE_COL)
        return TRUE;	
    if (sSlidingPuzzle->tiles[row1][cursor->sCol] == __ && row1 != FINAL_SLIDING_PUZZLE_ROW)
        return TRUE;
    if (sSlidingPuzzle->tiles[cursor->sRow][col1] == __ && col1 != FINAL_SLIDING_PUZZLE_COL)
        return TRUE;
...
static void SwapTile(void)
{
    ...
    if (sSlidingPuzzle->tiles[row0][cursor->sCol] == __ && row0 != FIRST_SLIDING_PUZZLE_ROW)
        sSlidingPuzzle->tiles[row0][cursor->sCol] = tile;
    else if (sSlidingPuzzle->tiles[cursor->sRow][col0] == __ && col0 != FIRST_SLIDING_PUZZLE_COL)
        sSlidingPuzzle->tiles[cursor->sRow][col0] = tile;	
    else if (sSlidingPuzzle->tiles[row1][cursor->sCol] == __ && row1 != FINAL_SLIDING_PUZZLE_ROW)
        sSlidingPuzzle->tiles[row1][cursor->sCol] = tile;
    else //if (sSlidingPuzzle->tiles[cursor->sRow][col1] == __  && col1 != FINAL_SLIDING_PUZZLE_COL)
        sSlidingPuzzle->tiles[cursor->sRow][col1] = tile;
	sSlidingPuzzle->tiles[cursor->sRow][cursor->sCol] = __;
