...
static const u16 sIsValidSpecies[] = {
	SPECIES_NONE, //SPECIES_DEOXYS, //SPECIES_NONE, // index 0 is used to tell when a daily wonder trade has already occurred, it should never be called
    //SPECIES_CHESPIN,
    //SPECIES_TEPIG,
    //SPECIES_POPPLIO,
    SPECIES_SPEAROW,
...
void TryUpdateWonderTrades(void)
{
	u16 index1;
	u16 index2;
	u16 index3;
	...
	do {
		index1 = Random() % NELEMS(sIsValidSpecies);
	} while (index1 == 0);
	
	do {
		index2 = Random() % NELEMS(sIsValidSpecies);
	} while (index2 == 0);
	
	do {
		index3 = Random() % NELEMS(sIsValidSpecies);
	} while (index3 == 0);
	
	do {
		index4 = Random() % NELEMS(sIsValidSpecies);
	} while (index4 == 0);
	
	do {
		index5 = Random() % NELEMS(sIsValidSpecies);
	} while (index5 == 0);
	
	do {
		index6 = Random() % NELEMS(sIsValidSpecies);
	} while (index6 == 0);
	
	VarSet(VAR_WONDER_TRADE_1, index1);
	VarSet(VAR_WONDER_TRADE_2, index2);
	VarSet(VAR_WONDER_TRADE_3, index3);
	VarSet(VAR_WONDER_TRADE_4, index4);
	VarSet(VAR_WONDER_TRADE_5, index5);
	VarSet(VAR_WONDER_TRADE_6, index6);
}
...
static u16 PickRandomSpecies(void) // picks only base forms
{
	...
	if (VarGet(VAR_WONDER_TRADE_1) != 0)
	{
		species = sIsValidSpecies[VarGet(VAR_WONDER_TRADE_1)];
		VarSet(VAR_WONDER_TRADE_1, 0);
	}
	else if (VarGet(VAR_WONDER_TRADE_2) != 0)
	{
		species = sIsValidSpecies[VarGet(VAR_WONDER_TRADE_2)];
		VarSet(VAR_WONDER_TRADE_2, 0);
	}
	else if (VarGet(VAR_WONDER_TRADE_3) != 0)
	{
		species = sIsValidSpecies[VarGet(VAR_WONDER_TRADE_3)];
		VarSet(VAR_WONDER_TRADE_3, 0);
	}
	else if (VarGet(VAR_WONDER_TRADE_4) != 0)
	{
		species = sIsValidSpecies[VarGet(VAR_WONDER_TRADE_4)];
		VarSet(VAR_WONDER_TRADE_4, 0);
	}
	else if (VarGet(VAR_WONDER_TRADE_5) != 0)
	{
		species = sIsValidSpecies[VarGet(VAR_WONDER_TRADE_5)];
		VarSet(VAR_WONDER_TRADE_5, 0);
	}
	else if (VarGet(VAR_WONDER_TRADE_6) != 0)
	{
		species = sIsValidSpecies[VarGet(VAR_WONDER_TRADE_6)];
		VarSet(VAR_WONDER_TRADE_6, 0);
	}
	else
		species = 0; // should never be called 
	//species = sIsValidSpecies[VarGet(VAR_0x8000)];
