...
/*static const u16 sSpeciesToNationalPokedexNum[NUM_SPECIES - 1] =
{
	...
	SPECIES_TO_NATIONAL(GOTHITURGE),
...
void CreateBoxMon(struct BoxPokemon *boxMon, u16 species, u8 level, u32 personality, struct OriginalTrainerId trainerId)
{
	...
	if (level <= MAX_LEVEL)
		...
		SetBoxMonData(boxMon, MON_DATA_EXP, &gExperienceTables[gSpeciesInfo[species].growthRate][0]);		
...
void CalculateMonStats(struct Pokemon *mon)
{
	...
	if (GetMonData(mon, MON_DATA_LEVEL, NULL) > MAX_LEVEL) // Only used for Ultra Space opponents
    ...
    for (i = 0; i < ITEM_EFFECT_ARG_START; i++)
    {
        ...
        case 3:
			...
			if (itemEffect[i] & ITEM3_SLEEP)
            {
				...
				{
					if (HealStatusConditions(mon, STATUS1_SLEEP, battler) == 0)
					{
				...
				if ((itemEffect[i] & ITEM3_PARALYSIS) && HealStatusConditions(mon, STATUS1_INFECT, battler) == 0)
					retVal = FALSE;
				else if ((itemEffect[i] & ITEM3_FREEZE) && HealStatusConditions(mon, STATUS1_DAZE, battler) == 0)
					retVal = FALSE;
				else if ((itemEffect[i] & ITEM3_BURN) && HealStatusConditions(mon, STATUS1_EXHAUST, battler) == 0)
					retVal = FALSE;
				else if ((itemEffect[i] & ITEM3_POISON) && HealStatusConditions(mon, STATUS1_FEAR, battler) == 0)
					retVal = FALSE;
            }
			...
			{
				if (HealStatusConditions(mon, STATUS1_PSN_ANY | STATUS1_TOXIC_COUNTER, battler) == 0 //&& (itemEffect[i] & ITEM3_POISON) && !(itemEffect[i] & ITEM3_SLEEP)
					&& ((itemEffect[i] == ITEM3_POISON) || (itemEffect[i] == ITEM3_STATUS_ALL)))
					retVal = FALSE;
				if (HealStatusConditions(mon, STATUS1_BURN, battler) == 0 //&& (itemEffect[i] & ITEM3_BURN) && !(itemEffect[i] & ITEM3_SLEEP)
					&& ((itemEffect[i] == ITEM3_BURN) || (itemEffect[i] == ITEM3_STATUS_ALL)))
					retVal = FALSE;
				if (HealStatusConditions(mon, STATUS1_FREEZE, battler) == 0 //&& (itemEffect[i] & ITEM3_FREEZE) && !(itemEffect[i] & ITEM3_SLEEP)
					&& ((itemEffect[i] == ITEM3_FREEZE) || (itemEffect[i] == ITEM3_STATUS_ALL)))
					retVal = FALSE;
				if (HealStatusConditions(mon, STATUS1_PARALYSIS, battler) == 0 //&& (itemEffect[i] & ITEM3_PARALYSIS) && !(itemEffect[i] & ITEM3_SLEEP)
					&& ((itemEffect[i] == ITEM3_PARALYSIS) || (itemEffect[i] == ITEM3_STATUS_ALL)))
					retVal = FALSE;
			}
            /*if ((itemEffect[i] & ITEM3_CONFUSION)  // heal confusion
             && gMain.inBattle && battler != MAX_BATTLERS_COUNT && (gBattleMons[battler].status2 & VOLATILE_CONFUSION))
            {
                gBattleMons[battler].status2 &= ~VOLATILE_CONFUSION;
                retVal = FALSE;
            }*/
            break;
