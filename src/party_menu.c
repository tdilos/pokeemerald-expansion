...
{
	// Put new statuses before old
	if (status & STATUS1_INFECT)
        return AILMENT_INF;
	if (status & STATUS1_DAZE)
        return AILMENT_DAZ;
	if (status & STATUS1_EXHAUST)
        return AILMENT_EXH;
	if (status & STATUS1_FEAR)
        return AILMENT_FER;
...
static void GetMedicineItemEffectMessage(u16 item, u32 statusCured)
{
	...
	case ITEM_EFFECT_CURE_INFECT:
        StringExpandPlaceholders(gStringVar4, gText_PkmnCuredOfInfection);
        break;
	case ITEM_EFFECT_CURE_DAZE:
        StringExpandPlaceholders(gStringVar4, gText_PkmnGotOverDaze);
        break;
	case ITEM_EFFECT_CURE_EXHAUST:
        StringExpandPlaceholders(gStringVar4, gText_PkmnCuredOfExhaustion);
        break;
	case ITEM_EFFECT_CURE_FEAR:
        StringExpandPlaceholders(gStringVar4, gText_PkmnGotOverFear);
        break;
...
u8 GetItemEffectType(u16 item)
{
    ...
    if (statusCure || (itemEffect[0] >> 7))
    {
		...
		else if (statusCure == ITEM3_INFECT)
            return ITEM_EFFECT_CURE_INFECT;
		else if (statusCure == ITEM3_DAZE)
            return ITEM_EFFECT_CURE_DAZE;
		else if (statusCure == ITEM3_EXHAUST)
            return ITEM_EFFECT_CURE_EXHAUST;
		else if (statusCure == ITEM3_FEAR)
            return ITEM_EFFECT_CURE_FEAR;
...
// Deletes all party mons, including eggs, except for the selected
