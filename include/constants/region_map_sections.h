//
// DO NOT MODIFY THIS FILE! It is auto-generated from src/data/region_map/region_map_sections.json and Inja template src/data/region_map/region_map_sections.constants.json.txt
//

#ifndef GUARD_CONSTANTS_REGION_MAP_SECTIONS_H
#define GUARD_CONSTANTS_REGION_MAP_SECTIONS_H

enum {
    MAPSEC_UMBER_TOWN,
    MAPSEC_CINEROUS_TOWN,
    MAPSEC_XANTHOS_CITY,
    MAPSEC_RESEDA_TOWN,
    MAPSEC_AMBER_CITY,
    MAPSEC_COBALT_CITY,
    MAPSEC_TURQUOISE_CITY,
    MAPSEC_MINDARO_CITY,
    MAPSEC_PITCHBLACK_CITY,
    MAPSEC_SCARLET_TOWN,
    MAPSEC_MAROON_CITY,
    MAPSEC_OFFWITE_TOWN,
    MAPSEC_HAEWEN_TOWN,
    MAPSEC_WISTERIA_CITY,
    MAPSEC_BURGUNDY_TOWN,
    MAPSEC_DUN_TOWN,
    MAPSEC_LITTLEROOT_TOWN,
    MAPSEC_OLDALE_TOWN,
    MAPSEC_DEWFORD_TOWN,
    MAPSEC_LAVARIDGE_TOWN,
    MAPSEC_FALLARBOR_TOWN,
    MAPSEC_VERDANTURF_TOWN,
    MAPSEC_PACIFIDLOG_TOWN,
    MAPSEC_PETALBURG_CITY,
    MAPSEC_SLATEPORT_CITY,
    MAPSEC_MAUVILLE_CITY,
    MAPSEC_RUSTBORO_CITY,
    MAPSEC_FORTREE_CITY,
    MAPSEC_LILYCOVE_CITY,
    MAPSEC_MOSSDEEP_CITY,
    MAPSEC_SOOTOPOLIS_CITY,
    MAPSEC_EVER_GRANDE_CITY,
    MAPSEC_ROUTE_101,
    MAPSEC_ROUTE_102,
    MAPSEC_ROUTE_103,
    MAPSEC_ROUTE_104,
    MAPSEC_ROUTE_105,
    MAPSEC_ROUTE_106,
    MAPSEC_ROUTE_107,
    MAPSEC_ROUTE_108,
    MAPSEC_ROUTE_109,
    MAPSEC_ROUTE_110,
    MAPSEC_ROUTE_111,
    MAPSEC_ROUTE_112,
    MAPSEC_ROUTE_113,
    MAPSEC_ROUTE_114,
    MAPSEC_ROUTE_115,
    MAPSEC_ROUTE_116,
    MAPSEC_ROUTE_117,
    MAPSEC_ROUTE_118,
    MAPSEC_ROUTE_119,
    MAPSEC_ROUTE_120,
    MAPSEC_ROUTE_121,
    MAPSEC_ROUTE_122,
    MAPSEC_ROUTE_123,
    MAPSEC_ROUTE_124,
    MAPSEC_ROUTE_125,
    MAPSEC_ROUTE_126,
    MAPSEC_ROUTE_127,
    MAPSEC_ROUTE_128,
    MAPSEC_ROUTE_129,
    MAPSEC_ROUTE_130,
    MAPSEC_ROUTE_131,
    MAPSEC_ROUTE_132,
    MAPSEC_ROUTE_133,
    MAPSEC_ROUTE_134,
    MAPSEC_UNDERWATER_124,
    MAPSEC_UNDERWATER_126,
    MAPSEC_UNDERWATER_127,
    MAPSEC_UNDERWATER_128,
    MAPSEC_UNDERWATER_SOOTOPOLIS,
    MAPSEC_GRANITE_CAVE,
    MAPSEC_MT_CHIMNEY,
    MAPSEC_SAFARI_ZONE,
    MAPSEC_BATTLE_FRONTIER,
    MAPSEC_PETALBURG_WOODS,
    MAPSEC_RUSTURF_TUNNEL,
    MAPSEC_ABANDONED_SHIP,
    MAPSEC_NEW_MAUVILLE,
    MAPSEC_METEOR_FALLS,
    MAPSEC_METEOR_FALLS2,
    MAPSEC_MT_PYRE,
    MAPSEC_AQUA_HIDEOUT_OLD,
    MAPSEC_SHOAL_CAVE,
    MAPSEC_SEAFLOOR_CAVERN,
    MAPSEC_UNDERWATER_SEAFLOOR_CAVERN,
    MAPSEC_VICTORY_ROAD,
    MAPSEC_MIRAGE_ISLAND,
    MAPSEC_CAVE_OF_ORIGIN,
    MAPSEC_SOUTHERN_ISLAND,
    MAPSEC_FIERY_PATH,
    MAPSEC_FIERY_PATH2,
    MAPSEC_JAGGED_PASS,
    MAPSEC_JAGGED_PASS2,
    MAPSEC_SEALED_CHAMBER,
    MAPSEC_UNDERWATER_SEALED_CHAMBER,
    MAPSEC_SCORCHED_SLAB,
    MAPSEC_ISLAND_CAVE,
    MAPSEC_DESERT_RUINS,
    MAPSEC_ANCIENT_TOMB,
    MAPSEC_INSIDE_OF_TRUCK,
    MAPSEC_SKY_PILLAR,
    MAPSEC_SECRET_BASE,
    MAPSEC_DYNAMIC,
    MAPSEC_SPECIAL_AREA,
    MAPSEC_AQUA_HIDEOUT,
    MAPSEC_MAGMA_HIDEOUT,
    MAPSEC_MIRAGE_TOWER,
    MAPSEC_BIRTH_ISLAND,
    MAPSEC_FARAWAY_ISLAND,
    MAPSEC_ARTISAN_CAVE,
    MAPSEC_MARINE_CAVE,
    MAPSEC_UNDERWATER_MARINE_CAVE,
    MAPSEC_TERRA_CAVE,
    MAPSEC_UNDERWATER_105,
    MAPSEC_UNDERWATER_125,
    MAPSEC_UNDERWATER_129,
    MAPSEC_DESERT_UNDERPASS,
    MAPSEC_ALTERING_CAVE,
    MAPSEC_NAVEL_ROCK,
    MAPSEC_TRAINER_HILL,
    MAPSEC_ROUTE_80,
    MAPSEC_ROUTE_81,
    MAPSEC_ROUTE_82,
    MAPSEC_ROUTE_83,
    MAPSEC_ROUTE_84,
    MAPSEC_ROUTE_85,
    MAPSEC_ROUTE_86,
    MAPSEC_ROUTE_87,
    MAPSEC_ROUTE_88,
    MAPSEC_ROUTE_89,
    MAPSEC_ROUTE_70,
    MAPSEC_ROUTE_71,
    MAPSEC_ROUTE_72,
    MAPSEC_ROUTE_73,
    MAPSEC_ROUTE_74,
    MAPSEC_ROUTE_75,
    MAPSEC_ROUTE_76,
    MAPSEC_ROUTE_77,
    MAPSEC_ROUTE_78,
    MAPSEC_ROUTE_79,
    MAPSEC_ROUTE_90,
    MAPSEC_ROUTE_91,
    MAPSEC_ROUTE_92,
    MAPSEC_ROUTE_93,
    MAPSEC_ROUTE_94,
    MAPSEC_ROUTE_95,
    MAPSEC_ROUTE_96,
    MAPSEC_ROUTE_97,
    MAPSEC_ROUTE_98,
    MAPSEC_ROUTE_99,
    MAPSEC_ROUTE_69,
    MAPSEC_ROUTE_68,
    MAPSEC_ROUTE_67,
    MAPSEC_ROUTE_66,
    MAPSEC_ROUTE_65,
    MAPSEC_UNDERWATER_73,
    MAPSEC_UNDERWATER_74,
    MAPSEC_KENDOMA_CORP,
    MAPSEC_SHUCKLE_SPRING,
    MAPSEC_QUARTZ_CAVE,
    MAPSEC_RESEDA_WOODS,
    MAPSEC_CINEROUS_TOWER,
    MAPSEC_SANDSHREAF,
    MAPSEC_DREAM_RESORT,
    MAPSEC_MINDARO_GARDENS,
    MAPSEC_MT_SHIRO,
    MAPSEC_SCARLET_WOODS,
    MAPSEC_BROADCAST_TOWER,
    MAPSEC_STEEL_MILL,
    MAPSEC_MIRAGE_PALACE,
    MAPSEC_ROYAL_REEF,
    MAPSEC_UNDERWATER_ROYAL_REEF,
    MAPSEC_POKEMON_LABORATORY,
    MAPSEC_DUSK_MOUNTAIN,
    MAPSEC_ISSHO_VICTORY_ROAD,
    MAPSEC_MERIDIA_TEMPLE,
    MAPSEC_AURORA_TEMPLE,
    MAPSEC_MEDIANOX_TEMPLE,
    MAPSEC_VESPER_TEMPLE,
    MAPSEC_POWER_STATION,
    MAPSEC_SHAMROCK_ISLAND,
    MAPSEC_UNDERGROUND_CITY,
    MAPSEC_ROUTE_100,
    MAPSEC_UNOWN_CHAMBER_1,
    MAPSEC_UNOWN_CHAMBER_2,
    MAPSEC_UNOWN_CHAMBER_3,
    MAPSEC_UNOWN_CHAMBER_4,
    MAPSEC_UNOWN_CHAMBER_5,
    MAPSEC_UNOWN_CHAMBER_6,
    MAPSEC_PERENNIAL_PILLAR,
    MAPSEC_DEWFORD_MEADOW,
    MAPSEC_STATION_CITY,
    MAPSEC_STATION_RING,
    MAPSEC_STATION_HULL,
    MAPSEC_MENTAL_MONASTERY,
    MAPSEC_DERELICT_SHIP,
    MAPSEC_PANTHALASSA,
    MAPSEC_OMAOMA_ISLAND,
    MAPSEC_KIYAHA_ISLAND,
    MAPSEC_KEOKEO_ISLAND,
    MAPSEC_PANO_ISLAND,
    MAPSEC_FORBIDDEN_FOREST,
    MAPSEC_FOREST_DALE,
    MAPSEC_SHIRE_TOWNE,
    MAPSEC_AVALON_CASTLE,
    MAPSEC_LUSH_UNDERSPIRE,
    MAPSEC_GREAT_SWAMP,
    MAPSEC_SWAMP_FACILITY,
    MAPSEC_MARTIAN_COLONY,
    MAPSEC_MARTIAN_DESERT,
    MAPSEC_CRYSTAL_MINE,
    MAPSEC_NONE,
    MAPSEC_COUNT
};

// Special location IDs that use the same value space as MAPSECs.
#define METLOC_SPECIAL_EGG         0xFD
#define METLOC_IN_GAME_TRADE       0xFE
#define METLOC_FATEFUL_ENCOUNTER   0xFF

#define KANTO_MAPSEC_START  MAPSEC_NONE //MAPSEC_PALLET_TOWN
#define KANTO_MAPSEC_END    MAPSEC_NONE //MAPSEC_SPECIAL_AREA
#define KANTO_MAPSEC_COUNT 0 //(KANTO_MAPSEC_END - KANTO_MAPSEC_START + 1)

#endif // GUARD_CONSTANTS_REGION_MAP_SECTIONS_H
